...
class QueueService
{
    ...
    public static function add($title, $uri, $later, array $data, $double = 1, $desc = '')
    {
        ...
        $jobId = Db::name('JobsLog')->insertGetId([
    ...
    {
        $result = Db::name('JobsLog')->where(['id' => $jobId])->update([
    ...
    public static function exists($title)
    {
        ...
        return Db::name('JobsLog')->where($where)->count() > 0;
    }
    ...
    {
        return Db::name('JobsLog')->where(['id' => $jobId])->find();
    }
    ...
    public static function del($jobId)
    {
        ...
        if (Db::name('JobsLog')->where($where)->delete() > 0) {
            Db::name('Jobs')->whereLike('payload', '%"_job_id_":"' . $jobId . '"%')->delete();
            return true;
        }
        return false;
    }
